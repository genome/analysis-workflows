...
outputs:
    ...
    stringtie_transcript_gtf:
        ...
        label: "Transcript GTF assembled from tumor RNA sample"
        doc: |
          GTF file containing the transcripts assembled from RNA-seq data created by StringTie
    stringtie_gene_expression_tsv:
        type: File
        outputSource: rnaseq/stringtie_gene_expression_tsv
        label: "Gene abundance table from tumor RNA sample"
        doc: |
          Tab-delimited output file containing gene abundances in FPKM and TPM, created by StringTie
    transcript_abundance_tsv:
        type: File
        outputSource: rnaseq/transcript_abundance_tsv
        label: "Transcript-level abundance table by kallisto"
        doc: |
          Tab-delimited output file containing the abundance estimates in the transcript level created by kallisto
    transcript_abundance_h5:
        type: File
        outputSource: rnaseq/transcript_abundance_h5
        label: "Transcript-level abundance data by kallisto in HDF5 file format"
        doc: |
          HDF5 binary file containing abundance esimates, bootstrap estimate, and so on, created by kallisto
    gene_abundance:
        type: File
        outputSource: rnaseq/gene_abundance
        label: "Gene-level abundance data by kallisto"
        doc: |
          Tab-delmited file containing the abundance estimates summarized in the gene level generated by Bioconductor tximport tool
    metrics:
        type: File
        outputSource: rnaseq/metrics
        label: "RNA-seq diagnostics/quality metrics"
        doc: |
          RNA-seq diagnostics/quality metrics describing the distribution of the bases within the transcripts, created by picard CollectRnaSeqMetrics tool
    chart:
        type: File?
        outputSource: rnaseq/chart
        label: "Plots for RNA-seq diagnostics/quality metrics"
        doc: |
          PDF file for the plot of normalized position vs. coverage as RNA-seq diagnostics/quality metrics, created by picard CollectRnaSeqMetrics
