      ...
      - entryname: 'docm_haplotypeCaller.sh'
        entry: |
             ...
             if [[ "$#" == 5 ]];then # If normal_bam is passed.
                 ...
                 cat $5 | grep '^@' > docm.interval_list # Extracting the header from the interval_list
                 zcat $4 | grep ^chr | awk '{FS = "\t";OFS = "\t";print $1,$2-100,$2+100,"+",$1"_"$2-100"_"$2+100}' >> docm.interval_list # Extracting the docm regions with a 100bp flanking region on both directions
                 ...
                 # Chaning the interval_list to a new docm_interval_list that spans the docm regions by 200bp
                 cat $4 | grep '^@' > docm.interval_list # Extracting the header from the interval_list
                 zcat $3 | grep ^chr | awk '{FS = "\t";OFS = "\t";print $1,$2-100,$2+100,"+",$1"_"$2-100"_"$2+100}' >> docm.interval_list # Extracting the docm regions with a 100bp flanking region on both directions
